...
def round_start_time(dt_utc: datetime, mode: str) -> datetime:
    ...
    if mode == "Minute la plus proche":
        ts2 = (ts + pd.Timedelta(seconds=30)).floor("min")
...
def main():
    ...
    st.title("SailGP – Départs (RACE_START_COUNT FRA) + Photo (fond de carte)")
    ...
    with st.sidebar:
        ...
        if mode.startswith("Auto"):
            st.caption("Arrondi appliqué à l’heure de départ utilisée pour le snapshot.")
    ...
    if mode.startswith("Auto"):
        with st.spinner("Lecture RACE_START_COUNT_unk (FRA), moyenne 1s, détection des incréments..."):
        ...
        if df_starts.empty:
            ...
            with st.expander("Debug – RACE_START_COUNT (FRA)", expanded=False):
                st.dataframe(df_count.head(1000), use_container_width=True)
            st.stop()
        ...
        start_time = round_start_time(detected_time_raw, round_mode)
    ...
    if mode.startswith("Auto"):
        st.subheader(
            f"Départ auto: détecté {detected_time_raw.strftime('%H:%M:%S')}Z → utilisé {start_time.strftime('%H:%M:%S')}Z | snapshot centré à T{offset_s:+d}s (±5s)"
        )
        st.caption(f"RACE_START_COUNT FRA {detected_info['count_before']:.0f} → {detected_info['count_after']:.0f}")
    else:
        st.subheader(
            f"Départ manuel: {start_time.strftime('%Y-%m-%d %H:%M:%S')}Z | snapshot centré à T{offset_s:+d}s (±5s)"
        )
