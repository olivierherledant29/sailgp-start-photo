...
CH_JIB_CUNNO_PCT = "PER_JIB_CUNNO_pct"
...
def prepare_df(df: pd.DataFrame, event_name: str) -> pd.DataFrame:
    ...
    return out.rename(
        columns={
            ...
            CH_JIB_CUNNO_PCT: "jib_cunno_per",
        }
    )
        ...
        fig.add_trace(
            go.Scattergl(
                ...
                hovertemplate=(
                    ...
                    "jib cunno %=%{x:.1f}<br>"
                    "jib cunno load=%{y:.0f} kgf<extra></extra>"
                ),
                customdata=np.stack(
                    [
                        d["boat"].astype(str),
...
def make_combined_html(day_dfs: dict[str, pd.DataFrame], df_all: pd.DataFrame, out_html: Path):
    ...
    subplot_titles = [
        f"{name} — jib cunno load vs jib cunno %"
        for name in event_names
    ] + ["ALL RACE DAYS — jib cunno load vs jib cunno %"]
    ...
    for i, event_name in enumerate(event_names, start=1):
        ...
        fig.update_yaxes(
            range=[1200, 2400],
            title_text="jib_cunno_load (kgf)",
            row=i,
            col=1,
        )
        fig.update_xaxes(
            title_text="jib_cunno_per (%)",
            row=i,
            col=1,
        )
    ...
    fig.update_layout(
        title=(
            "Jib cunno load vs jib cunno percentage — race days<br>"
            "<sup>Filters: target VMG > 80%, jib_cunno_load > 1200 kgf. "
...
def add_race_day_violins(
    ...
    *,
    ...
    - left: jib_cunno_load by team
    - right: jib_cunno_per by team
    ...
    for team in TEAM_ORDER:
        ...
        _add_violin_trace(
            ...
            col=1,
            team=team,
            values=d_team["jib_cunno_load"],
            metric_label="jib_cunno_load",
            showlegend=(row == 1),
        )
        ...
        _add_violin_trace(
            ...
            col=2,
            team=team,
            values=d_team["jib_cunno_per"],
            metric_label="jib_cunno_per",
            showlegend=False,
        )
...
def make_violins_html(
    ...
    Left: violin plots of jib_cunno_load by team.
    Right: violin plots of jib_cunno_per by team.
    ...
    for event_name in event_names:
        subplot_titles.extend(
            [
                f"{event_name} — jib cunno load density",
                f"{event_name} — jib cunno % density",
            ]
        )
    ...
    for row, event_name in enumerate(event_names, start=1):
        ...
        fig.update_yaxes(
            title_text="Jib cunno load (kgf)",
            row=row,
            col=1,
        )
        fig.update_yaxes(
            title_text="Jib cunno (%)",
            row=row,
            col=2,
        )
    ...
    fig.update_layout(
        title=(
            "Jib cunno density distributions by race day and team<br>"
            "<sup>"
            ...
            "Filters: target VMG &gt; 60%, jib cunno load &gt; 1200 kgf."
            "</sup>"
        ),
...
def main():
    ...
    out_dir = ROOT / "outputs" / "jib_cunno_race_days"
    ...
    if all_filtered:
        ...
        all_csv = out_dir / "all_race_days_jib_cunno_filtered.csv"
        combined_html = out_dir / "all_race_days_jib_cunno_plots.html"
        violins_html = out_dir / "all_race_days_jib_cunno_violins.html"
