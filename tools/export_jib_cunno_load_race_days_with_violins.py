...
def make_combined_html(day_dfs: dict[str, pd.DataFrame], df_all: pd.DataFrame, out_html: Path):
    ...
    subplot_titles = [
        f"{name} — jib cunno load vs BSP"
        for name in event_names
    ] + ["ALL RACE DAYS — jib cunno load vs BSP"]
    ...
    for i, event_name in enumerate(event_names, start=1):
        ...
        fig.update_yaxes(title_text="jib_cunno_load (kgf)", row=i, col=1)
        fig.update_xaxes(title_text="BSP (km/h)", row=i, col=1)

    all_row = n
    add_all_days_plot(fig, df_all, all_row)
    fig.update_yaxes(title_text="jib_cunno_load (kgf)", row=all_row, col=1)
    fig.update_xaxes(title_text="BSP (km/h)", row=all_row, col=1)

    fig.update_layout(
        title=(
            "Jib cunno load vs BSP — race days<br>"
            "<sup>Filters: target VMG &gt; 60%, jib cunno load &gt; 1200 kgf. "
    ...
    *,
    ...
    fig.add_trace(
        go.Violin(
            ...
            hovertemplate=(
                f"team={team}<br>"
                "jib cunno load=%{y:.2f} kgf<extra></extra>"
            ),
        ),
        row=row,
        col=col,
...
def make_violins_html(
    ...
    fig.update_layout(
        title=(
            "Jib cunno load density distributions by race day and team<br>"
            "<sup>"
