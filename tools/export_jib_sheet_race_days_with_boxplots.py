...
def _box_stats(values: pd.Series) -> dict | None:
    """
    Compute explicit box statistics.

    lowerfence and upperfence are the true minimum and maximum,
    so the whiskers represent min/max rather than Plotly's default 1.5 IQR.
    """
    ...
    *,
    ...
    fig.add_trace(
        go.Box(
            ...
            line=dict(
                color=color,
                width=2.4 if team == REF_BOAT else 1.4,
            ),
            ...
            hovertemplate=(
                ...
                f"n={stats['count']}<br>"
                f"min={stats['minimum']:.2f}<br>"
                f"Q1={stats['q1']:.2f}<br>"
                f"median={stats['median']:.2f}<br>"
                f"Q3={stats['q3']:.2f}<br>"
                f"max={stats['maximum']:.2f}<br>"
...
def add_race_day_boxplots(
    ...
    *,
    ...
    """
    Add two panels for one race day:
    - left: jib_sheet_load by team
    - right: jib_sheet_per by team

    TEAM_ORDER is forced on both axes, so missing teams keep an empty slot.
    """
    if df.empty:
        fig.add_annotation(
            text=f"{event_name}: aucune donnée filtrée",
    ...
    for team in TEAM_ORDER:
        ...
        load_stats = _box_stats(d_team["jib_sheet_load"])
        per_stats = _box_stats(d_team["jib_sheet_per"])
...
def make_boxplots_html(
    day_dfs: dict[str, pd.DataFrame],
    out_html: Path,
):
    """
    Create a second HTML page:
    one horizontal row per race day, with load and percentage boxplots.
    """
    event_names = list(day_dfs.keys())
    ...
    for event_name in event_names:
        subplot_titles.extend(
            [
                f"{event_name} — jib sheet load",
                f"{event_name} — jib sheet %",
            ]
        )
    ...
    fig.update_layout(
        title=(
            "Jib sheet distributions by race day and team<br>"
            "<sup>"
            "Each row is one race day. "
            "Boxes show Q1 / median / Q3; whiskers show true minimum and maximum. "
            "Empty team positions mean no filtered data. "
            "Filters: target VMG &gt; 60%, jib sheet load &gt; 1200 kgf."
            "</sup>"
        ),
        height=max(900, 480 * n_rows),
        template="plotly_white",
        boxmode="group",
        legend_title_text="Team",
...
def main():
    ...
    if all_filtered:
        ...
        boxplots_html = out_dir / "all_race_days_jib_sheet_boxplots.html"
        ...
        print(f"HTML boxplots : {boxplots_html}")
