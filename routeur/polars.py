...
def polar_twa_candidates(interp_or_table) -> list[float]:
    """Return a reasonable list of candidate TWA degrees for VMG search."""
    if isinstance(interp_or_table, PolarInterpolator):
        return [float(x) for x in interp_or_table.twa_grid]
    if isinstance(interp_or_table, pd.DataFrame):
        return [float(x) for x in interp_or_table.index.to_numpy(float)]
    return []
