...
# ----------------------------
# Generic mark conversion helpers (Routeur simplifié)
# ----------------------------

def marks_ll_to_xy(ctx, marks_ll: dict[str, tuple[float, float]]):
    """Convert marks dict {name: (lat, lon)} to {name: np.array([x,y])} using ctx transformers."""
    to_utm = ctx["to_utm"]
    out = {}
    for name, (lat, lon) in (marks_ll or {}).items():
        out[str(name)] = np.array(ll_to_xy(to_utm, float(lat), float(lon)), dtype=float)
    return out
